    ...
    else if (start == 0 && ready == 0)
    {
        ...
        for (int i = 0; i <= 2*iradius; i++) {
            for (int j = 0; j <= 2*iradius; j++) {
				...
				//(std(11) * (std(48) * (std(11) - std(11)) + std(48) * (std(11) - std(11))) - std(48)) * std(48)
				//(std(11) * (std(48) + std(48)
                ...
                if (rx > -1.0 && rx < (double) _IndexSize  &&
                    ...
                    num_i r = iy + (i - iradius) *step;// da li moze da se iskoriste prethodni dspejvi ali da se izlaz skrati na 10 downto 0
                    ...
                    if (r >= 1 + addSampleStep && r < _height - 1 - addSampleStep && c >= 1 + addSampleStep && c < _width - 1 - addSampleStep) {
                        ...
                        dxx = weight * (dxx1 - dxx2);//std(48)*std(48)=std(48)ali mora od 96-30-1 downto 18
                        dyy = weight * (dyy1 - dyy2);
                        dx = _cose * dxx + _sine * dyy;//std(48)*std(48) + std(48)*std(48) = std(48)
                        ...
                        rfrac = rx - ri;// std(48) - std(11) = std(48) 
						//ri nije unsigned jer pre ifa moze da bude manje od nule
                        cfrac = cx - ci;
                        ...
                        rweight1 = dx * (1.0 - rfrac); //std(48) * ( - std(48)) moze da se pretvori u signed i da se izracunaju operacije
			...
			if (ri >= 0 && ri < _IndexSize && ci >= 0 && ci < _IndexSize) {
			    index1D[ri * (_IndexSize * 4) + ci * 4 + ori1] += cweight1;//(std(11) * (std(48) * 4) + std(11) * 4 + std(11)) = (std(48) + std(11) + std(11))
				//std(48) += std(48) 
