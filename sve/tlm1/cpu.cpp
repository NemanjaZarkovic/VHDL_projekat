...
{
				SC_THREAD(software_dma);
    SC_THREAD(software_surf);
    ImLoad ImageLoader;
...
void Cpu::run() {
    // Execute both functions sequentially in one thread
    software_dma();
    software_surf();
}

void Cpu::software_dma()
{
    // Start the DMA process
    pl_t pl;
    pl.set_command(tlm::TLM_WRITE_COMMAND);
    pl.set_address(ADDR_DMA_START);
    pl.set_data_length(4);
    unsigned char buf[4] = {1};  // Start signal
    pl.set_data_ptr(buf);
    dma_socket->b_transport(pl, offset);  // Initiate DMA start

    // Wait for DMA to finish
    unsigned char result_buf[4];
    pl.set_command(tlm::TLM_READ_COMMAND);
    pl.set_address(ADDR_DMA_STATUS);
    pl.set_data_length(4);
    pl.set_data_ptr(result_buf);
    do {
        dma_socket->b_transport(pl, offset);  // Check DMA status
    } while (result_buf[0] != 0);  // Loop until DMA finishes

    // Print results after DMA finishes
    pl.set_address(ADDR_DMA_RESULT);
    for (int i = 0; i < ARRAY_SIZE / 2; ++i) {
								dma_socket->b_transport(pl, offset);
...
void Cpu::software_surf()
{
    ...
    // Get the length of the descriptor vector resulting from the parameterss
