        ...
        case TLM_READ_COMMAND:
            switch(addr)
            {
                case addr_ready:
                    ...
                    break;
               	case addr_ready_sum:
                    // If the request is for the summation process ready signal
                    intToUchar(buf, ready_sum);  // Return the summation ready status
                    break;
                default:
                    pl.set_response_status(TLM_ADDRESS_ERROR_RESPONSE);
            }
...
void Ip::proc() {
					while (true)
    {
        // Wait until two numbers are available in the input FIFO
        if (p_fifo_in->num_available() >= 2 && ready_sum == 1)  // Use the ready_sum signal here
        {
            ready_sum = 0;  // Set ready_sum to 0 while summation is in progress

            // Read two numbers from the FIFO
            num1 = p_fifo_in->read();
            num2 = p_fifo_in->read();
            ...
            p_fifo_out->write(result);

            // Simulate processing delay
            wait(sc_time(10, SC_NS));

            ready_sum = 1;  // Set ready_sum back to 1 to indicate readiness for the next summation
        }
        else
        {
            wait(p_fifo_in->data_written_event());  // Wait for more data in the FIFO
        }
    }
