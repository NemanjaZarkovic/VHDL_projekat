...
void doubleToTwoInts(int32_t &intPart, int32_t &fracPart, double val) {
    // Skaliramo vrednost tako da dobijemo preciznost u celim brojevima (30 bita za ceo broj i 18 bita za frakcioni deo)
    const int64_t scaleFactor = 1LL << 18; // 2^18 za preciznost frakcionog dela
    
    // Prvo ćemo izdvojiti celobrojni deo
    int64_t scaledValue = static_cast<int64_t>(val * scaleFactor); // Skaliramo vrednost kako bismo dobili i frakcioni deo
    intPart = static_cast<int32_t>(scaledValue >> 18); // 30-bitni celobrojni deo
    fracPart = static_cast<int32_t>(scaledValue & ((1 << 18) - 1)); // 18-bitni frakcioni deo
}
