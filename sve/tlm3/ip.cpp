...
void Ip::b_transport(pl_t& pl, sc_time& offset)
{
    ...
    switch (cmd)
    {
        case TLM_WRITE_COMMAND:
            ...
            {
            				case ADDR_IP_INPUT: {
                    // Receive two numbers and store them
                    broj1 = toInt(buf);           // First 4 bytes
                    broj2 = toInt(buf + 4);       // Next 4 bytes
                    ready = 0;                   // Indicate that the IP is processing
                    cout << "Received numbers: " << broj1 << " and " << broj2 << endl;
                    break;
                    }
                case addr_start:
                    start = toInt(buf);
        ...
        case TLM_READ_COMMAND:
            switch(addr)
            {
                    ...
                    intToUchar(buf, result);  // Write the result back to the buffer
                    ready = 1;  // Indicate that IP is ready again
